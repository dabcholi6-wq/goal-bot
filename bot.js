...
bot.start((ctx) => {
    ...
    if (chatType === 'group' || chatType === 'supergroup') {
        ctx.getChatMember(ctx.botInfo.id).then((botMember) => {
            ...
            ctx.reply('❌ خطا در بررسی وضعیت ادمین. لطفاً دوباره تلاش کن.');
        });
        return;
    }

    // اگر کاربر در پیوی است
    getUser(userId, (user) => {
...
// ==========================================
// 🎮 بازی شرط‌بندی (بدون محدودیت گروه)
// ==========================================
const games = {};
...
bot.hears(/^بازی (\d+)$/, (ctx) => {
    ...
    getUser(userId, (user) => {
        ...
        updateUser(userId, { diamonds: user.diamonds - amount }, () => {
            ...
            ctx.reply(
                `🎲 بازی با ${amount} الماس شروع شد!\n` +
                `💰 جایزه برنده: ${amount * 2} الماس\n\n` +
    ...
    getUser(userId, (user) => {
        ...
        updateUser(userId, { diamonds: user.diamonds - game.amount }, () => {
            ...
            getUser(winner, (winnerUser) => {
                updateUser(winner, { diamonds: winnerUser.diamonds + game.prize }, () => {
                    getUser(winner, (finalWinner) => {
                        getUser(loser, (finalLoser) => {
                            ctx.reply(
                                `🏅 **برنده مسابقه:** [${finalWinner.user_id}]\n` +
                                `❌ **بازنده مسابقه:** [${finalLoser.user_id}]\n\n` +
                                `💰 جایزه: ${game.prize} الماس`,
                                {
                                    reply_markup: {
                                        inline_keyboard: [
                                            [{ text: `💎 جایزه: ${game.prize}`, callback_data: 'dummy' }],
                                            [{ text: `💰 موجودی برنده: ${finalWinner.diamonds}`, callback_data: 'dummy' }],
                                            [{ text: `💸 موجودی بازنده: ${finalLoser.diamonds}`, callback_data: 'dummy' }]
                                        ]
                                    }
                                }
                            );
...
bot.hears(/^بازی (\d+) با ربات$/, (ctx) => {
    ...
    getUser(userId, (user) => {
        ...
        updateUser(userId, { diamonds: user.diamonds - amount }, () => {
            const winner = Math.random() < 0.5 ? 'user' : 'bot';

            if (winner === 'user') {
                getUser(userId, (updatedUser) => {
                    const newDiamonds = updatedUser.diamonds + (amount * 2);
                    updateUser(userId, { diamonds: newDiamonds }, () => {
                        ctx.reply(
                            `🏅 **برنده:** شما\n` +
                            `❌ **بازنده:** ربات\n\n` +
                            `💰 جایزه: ${amount * 2} الماس`,
                            {
                                reply_markup: {
                                    inline_keyboard: [
                                        [{ text: `💎 جایزه: ${amount * 2}`, callback_data: 'dummy' }],
                                        [{ text: `💰 موجودی شما: ${newDiamonds}`, callback_data: 'dummy' }]
                                    ]
                                }
                            }
                        );
                ...
                ctx.reply(
                    ...
                    `💰 جایزه: ${amount * 2} الماس`,
                    {
                        reply_markup: {
                            inline_keyboard: [
                                [{ text: `💎 جایزه: ${amount * 2}`, callback_data: 'dummy' }],
                                [{ text: `💰 موجودی شما: ${user.diamonds - amount}`, callback_data: 'dummy' }]
                            ]
                        }
                    }
                );
...
// ==========================================
// 📁 دکمه بی‌کاربرد (برای نمایشی)
// ==========================================
bot.action('dummy', (ctx) => ctx.answerCbQuery());
